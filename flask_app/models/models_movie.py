...
# Now we use class methods to query our database
    ...
    def save(cls, data):
        ...
        query = "INSERT INTO movies ( movie_id, imdb_id, title, year, release_date, tagline, overview, poster_path, original_language, rating, user_rating, user_id, created_at, updated_at ) VALUES (%(movie_id)s, %(imdb_id)s,  %(title)s, %(year)s, %(release_date)s, %(tagline)s,  %(overview)s, %(poster_path)s, %(original_language)s, %(rating)s,  %(user_rating)s, %(user_id)s, NOW(), NOW());"
    ...
    @classmethod
    def update(cls,data):
        query = "UPDATE movies SET movie_id=%(movie_id)s, imdb_id=%(imdb_id)s, title=%(title)s, year=%(year)s, release_date=%(release_date)s, tagline=%(tagline)s, overview=%(overview)s, poster_path=%(poster_path)s, original_language=%(original_language)s, user_rating=%(user_rating)s, updated_at = NOW() WHERE id = %(id)s;"
        return connectToMySQL(DATABASE).query_db(query,data)
    ...
    def get_poster(cls, data):
        query = "SELECT poster_path FROM movies WHERE movies.id = %(id)s;"
        results = connectToMySQL(DATABASE).query_db(query,data)
